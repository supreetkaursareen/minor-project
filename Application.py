    ...
    def video_loop(self):
        ...
        if ok:
            ...
            if (len(predicts) > 1):
                ...
                self.bt1.config(text=predicts[0], font=("Courier", 20),state=tk.NORMAL)

            else:

                self.bt1.config(text="", state=tk.DISABLED)

            if (len(predicts) > 2):

                self.bt2.config(text=predicts[1], font=("Courier", 20),state=tk.NORMAL)

            else:

                self.bt2.config(text="", state=tk.DISABLED)

            if (len(predicts) > 3):

                self.bt3.config(text=predicts[2], font=("Courier", 20),state=tk.NORMAL)

            else:

                self.bt3.config(text="", state=tk.DISABLED)
